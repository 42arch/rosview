...
`getRosViewMessages()` in [`src/shared/intl/loadRosViewMessages.ts`](src/shared/intl/loadRosViewMessages.ts) merges shards at build time; [`src/features/panels/framework/panelMessageSlug.ts`](src/features/panels/framework/panelMessageSlug.ts) documents `PanelType` → message slug mapping for `panels.<slug>.*` keys.
...
Open a [GitHub Discussion](https://github.com/ioai-tech/rosview/discussions) if you have questions before opening an issue or PR.

**Note:** The open-source code and npm package live under the [`ioai-tech`](https://github.com/ioai-tech) GitHub organization. Security and conduct contacts may use `@io-ai.tech` email addresses; that domain is operated by the same maintainers as **IO-AI Tech** (see `LICENSE`).
