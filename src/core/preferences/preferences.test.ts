...
import { ROS_VIEW_PREFERENCE_SCHEMA_VERSION } from './types';
...
describe('readPreferences / writePreferences', () => {
  ...
  it('returns null for empty or corrupt storage', () => {
    ...
    localStorage.setItem(ROS_VIEW_PREFERENCES_STORAGE_KEY, 'not-json');
    expect(readPreferences()).toBe(null);
    localStorage.setItem(ROS_VIEW_PREFERENCES_STORAGE_KEY, JSON.stringify({ theme: 'dark' }));
    expect(readPreferences()).toBe(null);
